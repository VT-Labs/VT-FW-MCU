 ...
 * @param [in]   dataLen - length of data.
 * @param [in]	 *databuff - is data buffer.
 * @param [in]	 level - is alert level of a malicious frame.
 * @return        status
 */
vt_status_t vt_fw_add_malicious_can_frame(uint32_t msgId, uint8_t dataLen, uint8_t *databuff, uint8_t level);
 ...
 * @param [in]	 level - is alert level of a malicious range id.
 * @return       status
 */
vt_status_t vt_fw_blacklist_add_range_can_id(uint32_t fromId, uint32_t toId, uint8_t operator, uint8_t level);
